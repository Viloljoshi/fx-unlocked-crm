...
export async function GET(request) {
  try {
    ...
    // Check deal status — can't reject if already active or rejected
    ...
    if (deal.status === 'ACTIVE') {
      return NextResponse.json({ error: 'This deal has already been approved and cannot be rejected', code: 'ALREADY_USED' }, { status: 409 })
    }
    if (deal.status === 'REJECTED') {
      return NextResponse.json({ error: 'This deal has already been rejected', code: 'ALREADY_USED' }, { status: 409 })
    }

    // Update deal to REJECTED
    await supabase
      .from('deals')
    ...
    try {
      ...
      await supabase.from('deal_versions').insert({
        ...
        changes: { action: 'REJECTED', method: 'email_link', ip },
      ...
      await supabase.from('audit_logs').insert({
        ...
        changes: { status: { from: deal.status, to: 'REJECTED' }, method: 'email_link' },
      })
